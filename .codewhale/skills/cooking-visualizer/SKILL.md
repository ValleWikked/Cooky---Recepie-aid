...
description: >
  ...
  when the user says "визуализируй", "разбор", "покажи этапы", or any similar phrase
  after a recipe has been given or discussed. Also trigger when the user says
  "разбор рецепта" or "покажи ингредиенты" at any point in a cooking conversation.
...
Label them with necessity tier `УСЛОВНЫЙ` and state the error they compensate for:

- Мука in falafel: not traditional. Needed only if mixture fails the ball test
  due to excess moisture from incorrect drying or over-processed nut. Label:
  "страховка — нужна только если смесь не держит форму".
- Разрыхлитель in falafel: not traditional. More justified in an air fryer than
  in deep-fry (hot oil creates explosive steam lift that air fryers lack). Label:
  "западная адаптация — делает текстуру чуть легче, не обязателен".

**Rule 3 — Verified equipment specs.**

- Kenwood FDP22.130GY: чаша **2.1 L** — not 1.5 L. Do not repeat the 1.5 L error.
- Ninja MAX PRO: 6.2 L, max 240°C. Air fry times are always ESTIMATED (~).
  Always instruct the user to calibrate on the first batch.

**Rule 4 — No conflation of different dishes.** Ta'amia (та'амия) is Egyptian
falafel made from fava beans (конские бобы), not chickpeas. Chickpea falafel
...
- `ОСНОВА` — the dish structurally or chemically fails without it.
- `ВАЖНО` — omitting significantly changes the character; the dish still works.
- `ОПЦИОНАЛЬНО` — omitting is fully acceptable; this is flavour or texture
  enhancement only.
- `УСЛОВНЫЙ` — present in western adaptations to compensate for technique errors;
...
- Соль: both ¾× and ⁵⁄₄× are already noticeably wrong. Very narrow zone.
- Разрыхлитель: ⁵⁄₄× causes structural collapse and soapy taste. Narrow upper bound.
- Мука (as binder): mark the entire 1× column as `?` — the correct amount is
  empirically determined by the ball test, not by a fixed number.

**Forgiving ingredients** have wide zones:
- Чёрный перец: the zone spans roughly ¾× to ⁵⁄₄× without meaningful difference.
- Петрушка/кинза: wide zone below 1×; moderate zone above (moisture risk at 3/2×).
...
A collapsible sub-section titled "Варианты и замены" showing:
...
4. **Equipment settings** — shown as compact chips in the collapsed card:
   ...
   - No device: "вручную" + brief technique note
...
instruction naming the specific check: e.g. "проверь первую партию на 11 мин —
разломи один шарик; если внутри тестообразно — добавь 2 мин". This instruction
...
with a warning indicator. "Развернуть все" toggle at panel level.
...
**Tabs:** "Ингредиенты" and "Этапы приготовления" are mandatory. Third tab only
...
**Interactivity:** collapsible rows in both panels, "развернуть все" toggle per
...
Single .jsx file. All units in metric: г, мл, °C.
...
1. Identify the base ingredient(s). Two co-equal bases → both marked ОСНОВА,
   both excluded from the matrix.
2. List all other ingredients with exact amounts. For each, assign necessity tier
   honestly — distinguish ВАЖНО from УСЛОВНЫЙ carefully.
...
4. Enumerate all steps in chronological, granular order. Сушка and измельчение
   are separate steps; охлаждение and лепка are separate steps.
...
6. Run a final check against the four Core Accuracy Rules:
   ...
   - No 1.5 L bowl.
   - No unconditional мука/разрыхлитель presentation.
